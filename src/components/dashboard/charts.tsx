...
export function ExpensePieChart({ data }: { data: CategorySummary[] }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <PieChart>
        <Pie data={data} dataKey="total" nameKey="categoryName" cx="50%" cy="50%" outerRadius={80} label={({ categoryName, percentage }) => `${categoryName} ${percentage.toFixed(0)}%`} labelLine={false}>
          {data.map((entry, i) => (
